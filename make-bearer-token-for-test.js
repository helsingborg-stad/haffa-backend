...
const getIndexedArgPlusOne = (args, index) =>
  index === -1 ? null : args[index + 1]
const getNamedArg = (args, name) =>
  getIndexedArgPlusOne(args, args.indexOf(name))
...
const main = () => {
  ...
  if (!sharedSecret) {
    console.error(
      'Environment variable JWT_SHARED_SECRET is missing. Aborting.'
    )
    process.exit(1)
  }
  ...
  if (!id) {
    console.log(
      `Usage: node ${path.basename(__filename)} --id <person to impersonate>`
    )
    process.exit(2)
  }
