...
export const transact = async <T extends { versionId: string }>(
  ctx: TxCtx<T>
  ...
  async function nextAttempt(
    ...
    await Promise.all(
      (commitActions || [])
        ...
        .map(action => action(data, original))
    )
...
export interface TxBuilder<T> {
  ...
  saveVersion: (
    saveVersion: TxCtx<T>['saveVersion']
  ) => Pick<TxBuilder<T>, 'run'>
  run: (opts?: {
    maxRetries?: number
    retryDelay?: number
  }) => Promise<TxResult<T> | null>
...
export const txBuilder = <T extends { versionId: string }>(): Pick<
  TxBuilder<T>,
  'load'
> => {
  const builder = (ctx: TxCtx<T>): TxBuilder<T> => ({
    ...
    run: ({ maxRetries, retryDelay } = {}) =>
      transact({ ...ctx, maxRetries, retryDelay }),
  })

  const notset = (method: string) => (): never => {
    throw new Error(`${method}() must be set in transaction`)
  }
  return builder({
    load: notset('load'),
