...
const profilesTest = (testfn: ((profiles: ProfileRepository, connection: MongoConnection<MongoProfile>) => Promise<any>)): Promise<any> => mongoTest(
...
describe('mongo profile respository', () => {
	it('getProfile() returns blank user if missing in db', () => profilesTest(async profiles => {
	...
	it('updateProfile() upserts profile in db', () => profilesTest(async profiles => {
	...
	it('updateProfile() overwrites profile in db', () => profilesTest(async (profiles, connection) => {
		const user = createTestUser({id: 'donald@duck.com'})
		const input = createProfileInput({
			phone: '555-123456',
			city: 'Duck City',
