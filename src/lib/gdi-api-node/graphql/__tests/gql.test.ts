...
const createAuthorizationHeadersFor = (
  id: string,
...
describe('GraphQL', () => {
  it('POST /api/v1/test/graphql handles combinations of sync/async/promise resolvers', () =>
    createTestApp(TEST_SHARED_SECRET).run(async server => {
      const {
        status,
        body: { data, errors },
  ...
  it('POST /api/v1/test/graphql validates parameters ({query, variables} = body)', () =>
    createTestApp(TEST_SHARED_SECRET).run(async server => {
      const {
        status,
        body: { data },
  ...
  it('POST /api/v1/test/graphql requires valid authorization header', () =>
    createTestApp(TEST_SHARED_SECRET).run(async server => {
      const { status } = await request(server)
        .post('/api/v1/test/graphql')
  ...
  it('POST /api/v1/test/graphql can utilize cache in implementation', () =>
    createTestApp(TEST_SHARED_SECRET).run(async server => {
      const {
        status,
        body: { data },
