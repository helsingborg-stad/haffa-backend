...
export const createMongoSettingsService = ({getCollection}: MongoConnection<MongoSetting>): SettingsService => {
	const getSetting: SettingsService['getSetting'] = async <T>(id: string): Promise<T|null> => getCollection()
		.then(collection => collection.findOne({id}))
		.then(setting => (setting?.value || null) as T)

	const updateSetting: SettingsService['updateSetting'] = <T>(id: string, value: T): Promise<T|null> => getCollection()
