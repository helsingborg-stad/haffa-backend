...
export const verifyTypeIsReservation = ({
  update,
  throwIf,
}: TxVerifyContext<Advert>): void =>
  throwIf(update.type !== AdvertType.recycle, {
    code: 'EADVERT_',
    message: 'Endast återbruksannonser kan reserveras',
  })

export const verifyQuantityAtleatOne = ({
  update,
  throwIf,
}: TxVerifyContext<Advert>): void =>
  throwIf(!(update.quantity >= 1), {
    code: 'EADVERT_',
    message: 'Antalet måste vara minst 1',
    field: 'quantity',
  })

export const verifyReservationsDoesNotExceedQuantity = ({
  update,
  throwIf,
}: TxVerifyContext<Advert>): void =>
...
export const verifyReservationLimits = ({
  update,
  throwIf,
}: TxVerifyContext<Advert>): void =>
