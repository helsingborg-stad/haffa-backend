...
import { readdir, readFile, stat as _stat, writeFile } from 'fs/promises'
...
export const createFsAdvertsRepository = (
  ...
  const getAdvert: AdvertsRepository['getAdvert'] = async (id: string) =>
  ...
  const list: AdvertsRepository['list'] = async filter =>
    readdir(dataFolder)
      .then(names => names.filter(name => /.*\.json$/.test(name)))
      .then(names => names.map(name => join(dataFolder, name)))
      .then(paths =>
        Promise.all(
          paths.map(path => _stat(path).then(stat => ({ stat, path })))
        )
      )
      .then(stats =>
        Promise.all(
  ...
  const update: AdvertsRepository['update'] = async (id, _, input) => {
